...
namespace PayrollSystem
{
    public partial class SSSForm : Form
    {
        ...
        {
            AddSSSForm addsss = new AddSSSForm();
            addsss.Show();
        }
        ...
        {
             DialogResult dialogResult = MessageBox.Show("Do you want to save the changes?", "SSS Table Confirmation", MessageBoxButtons.YesNo);
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Do you want to delete the selected row?", "Deleting SSS Row", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                ...
                string constring = "datasource=localhost;port=3306;username=root;password=toor";
                string query = "DELETE from db_payroll.tblsss WHERE sssID ='" + getID + "';";
                MySqlConnection conDataBase = new MySqlConnection(constring);
                MySqlCommand cmdDataBase = new MySqlCommand(query, conDataBase);
                MySqlDataReader myReader;
                ...
                try
                {
                    ...
                    loadTable();
                    MessageBox.Show("SSS Table has successfully updated!");
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
